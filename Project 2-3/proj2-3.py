...
#   that can come from a large dataset like Buckeye. It is also interesting that this speech
#   continues to be Zipfian despite being spoken rather than written language.
# To Execute: python3 proj2-3.py <number for tokenization>
...
# Pre: current directory contains a directory called buckeye
#   which contains text files
# Post: BSC.txt is created and written containing buckeye corpus
def compile_corpus():
    fout = open('BSC.txt', 'w')
    os.chdir('buckeye')
    ...
    for file_name in file_names:
        fin = open(file_name, 'r', encoding='utf8')
        text = fin.read()
        fout.write(text)
        fout.write('\n')
...
# Uncomment to get first compilation of buckeye corpus into BSC.txt
# compile_corpus()
main()